  ...
  <div class="relative md:flex p-2 ">
    ...
    <div class=" overflow-hidden sm:block md:hidden">
      <a-carousel arrows autoplay>
        ...
        <template #nextArrow>
          <div class="custom-slick-arrow" v-if="currentPage <= (posts.length / pageSizeMobile) && !loading" style="right:10px"
            @click="scrollRightMobile">
            <right-circle-outlined />
          </div>
        </template>
        <div class="relative w-full " v-for="(post, index) in posts" :key="index">
          <router-link :to="{ name: 'PostDetail', params: { postId: post.id } }">
            <!-- <div class="relative  w-full">
              ...
              <div
                class="absolute bottom-0 left-0 right-0 top-0 z-1 flex flex-col justify-start p-2 overflow-hidden  bg-opacity-30 bg-gray-100 w-full">
                <span class="flex justify-start font-normal font-mono text-gray-700   text-2xl mt-20 w-full ">{{
                  truncateText(post.title, 2) }}</span>
    ...
    <div class="ml-0 md:flex flex-row list overflow-hidden">
    ...
    <!-- <div v-if="currentPage <= (posts.length / pageSizeMobile)"
      class="bg-gray-50 bg-opacity-80 text-black font-bold text-4xl rounded-full absolute top-0 right-0 mr-0 h-12 w-11 mt-24 z-2 flex items-center justify-center md:hidden"
      @click="scrollRightMobile">
      <RightOutlined />
    </div> -->
...
export default {
  ...
  setup() {
    ...
    function getMyPost() {
      ...
      API.trendingProductPost(filterField)
        .then((res) => {

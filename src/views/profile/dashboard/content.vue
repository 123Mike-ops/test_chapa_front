<template>
    <div class="p-2 md:flex flex-col w-full">
      ...
      <div class="">  <a-divider  orientation="left">Ads On This Category</a-divider></div>
      ...
      <div v-if="store.state.user.authenticated  && user?.id!==store.state.user.userData?.id && amIFollowing==false" class="flex flex-col bg-white w-52 p-5 rounded-md">
      ...
      <div v-else-if="store.state.user.authenticated  && user?.id!==store.state.user.userData?.id && amIFollowing" class="flex flex-col bg-white w-52 p-5 rounded-md mt-2">
  ...
  <script>
  import { UnorderedListOutlined, TableOutlined, SortDescendingOutlined, ClockCircleOutlined,UserOutlined } from "@ant-design/icons-vue";
    ...
    methods: {
      ...
      clearForm(value) {
        ...
        if (value == 'sort') {
          ...
          this.filterField.userId=this.user?.id;
